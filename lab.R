...
#' According to the description for the NYT county-level data: "When the information is available, we count patients where they are being treated, not necessarily where they live."
#' Further, non-standardized reporting methods across the country at all levels - federal, state, local - resulted in the journalists who compiled this data having to make judgement calls about how to count and report cases.
#' 

#' 2. *How about the Google Mobility data?* 
#' It seems to be restricted to Apple Map data
#' 
...
#' The outbreaks appear to have some peaks in July, but also throughout August and still into September. It does not appear that things have faded.
#' 
#' 
...
skimr::skim(mob_df)
#' There looks to be 11 distinct types. The variable looks to be a classifier for what kind of district is being referenced.
#' 

#' 2. *How about `pct_diff`?* 
#' It's the percentage difference in mobility in those types of areas compared to baseline
#' 
#' 
...
count(mob_df, county)
#' There seems to be 2 counties missing mobility data.
#' 
...
#' The graphs seem to support that narrative of people staying home in Spring and then going out more, at least to park areas, in Summer.
#' 
#' 

#' 6. *What other potentially interesting patterns do you see in these mobility data?* 
#' It's interesting to me that Merced seems to have a greater increase of mobility in park areas compared to the other counties we're focused on.
#' 
...
#' Rather than using month-long averages, week-long averages throughout June and July might give a finer-grained understanding of the relationship between the relaxation of stay-at-home suggestions and any subsequent outbreaks.
#' 
#' 
...
parks_june <- mob_df %>% 
  ...
  group_by(county, fips) %>% 
  summarize(parks = mean(pct_diff, na.rm = T)) %>% 
  ungroup() %>% 
  filter(!is.na(parks))
...
summer_df <- inner_join(cases_july, parks_june, by = c('county','fips'))
...
#' It doesn't look like the scatterplot supports this. Unless I'm  misunderstanding what the pct_diff variable represents, or have done this assignment wrong (both strong possibilities!), it looks like cases per pop in July is negatively correlated with people spending more time in parks in June.
#' 
